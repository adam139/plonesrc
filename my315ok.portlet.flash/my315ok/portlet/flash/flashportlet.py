...
try:
    ...
    class IFlashMovie(Interface):
        "mark interface for flash object"
...
#        portal_state = getMultiAdapter((self.context, self.request), name=u'plone_portal_state')
    ...
    def js_settings(self):
        ...
        if swf:
            svars = self.swfvars()
            out = """genswf("%(swfile)s","%(we)s",%(ht)s,%(wh)s,"#%(swfLocid)s");""" \
            % dict(swfile=swf,ht=svars['height'],wh=svars['width'],we=svars['wmode'],swfLocid=self.locid())
            return out                       
        else:
